...
CREATE TABLE LoggingEmpleados(
    idLogEmpleado int not null AUTO_INCREMENT,
    user1 char(56),
    correo char(30),
    pasword char(56),
...
CREATE TABLE Empleados(
    ...
    CONSTRAINT tiene FOREIGN KEY(idEmpleado) REFERENCES LoggingEmpleados(idLogEmpleado) ON DELETE CASCADE
    );

    CREATE TABLE CanchasFutbol(
    idfutbol int not null AUTO_INCREMENT,
    precio int (4),
    PRIMARY KEY(idfutbol)
    );
    INSERT INTO `canchasfutbol`(`precio`) VALUES (700);
    INSERT INTO `canchasfutbol`(`precio`) VALUES (700);

    CREATE TABLE CanchasPaddle(
    idPaddle int not null AUTO_INCREMENT,
    precio int (4),
    PRIMARY KEY(idPaddle)
   );
    INSERT INTO `canchaspaddle`(`precio`) VALUES (700);
    INSERT INTO `canchaspaddle`(`precio`) VALUES (700);
    INSERT INTO `canchaspaddle`(`precio`) VALUES (700);


    CREATE TABLE Pileta(
    idPileta int not null AUTO_INCREMENT,
    precio int(4),
    cantidadUsuarios int(2) DEFAULT 10,
    PRIMARY KEY(idPileta)
   );
    INSERT INTO pileta(precio) VALUES(600);
    ...
    INSERT INTO gym(precio) VALUES(600);

    CREATE TABLE GruposReserba(
    idGrupo INT AUTO_INCREMENT,
    hora INT(2),
    fecha DATE,
    deporte CHAR(15),
    contador int(10) DEFAULT 0,
    PRIMARY KEY(idGrupo)
    );
    ...
    nombre char(15),
    apellido char(15),
    formaPago char(10) DEFAULT "Efectivo",
    totalPagar int(5) DEFAULT 0,
    ...
    paddle int,
    grupo int,
    CONSTRAINT  empleado  FOREIGN KEY(empleado) REFERENCES empleados(idEmpleado) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT  futbol  FOREIGN KEY(futbol) REFERENCES CanchasFutbol(idfutbol) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT  paddle  FOREIGN KEY(paddle) REFERENCES CanchasPaddle(idPaddle) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT  pileta  FOREIGN KEY(pileta) REFERENCES Pileta(idPileta) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT  gym  FOREIGN KEY(gym) REFERENCES Gym(idGym) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT  grupos  FOREIGN KEY(grupo) REFERENCES GruposReserba(idGrupo) ON DELETE CASCADE ON UPDATE CASCADE
    );
    ...
    produc int,
    empleado int,
    CONSTRAINT empleado1 FOREIGN KEY(empleado) REFERENCES empleados(idEmpleado)ON DELETE CASCADE,
    CONSTRAINT producto1 FOREIGN KEY(produc) REFERENCES productos(idProducto) ON DELETE CASCADE
);
...
CREATE FUNCTION INSERTAR_CLIENTE_GYM(EMPLEADO INT (4),NOMBRE_CLIENTE CHAR(15),APELLIDO_CLIENTE CHAR(15),DEPORTE_CLIENTE CHAR(9),FECHA_CLIENTE DATE,HORA_CLIENTE INT(2)) RETURNS VARCHAR(100)
    ...
    DECLARE CONTADOR1 INT(5) DEFAULT (SELECT gruposreserba.contador FROM gruposreserba WHERE gruposreserba.hora = HORA_CLIENTE AND gruposreserba.fecha = FECHA_CLIENTE AND gruposreserba.deporte = 'gym');
    ...
    SET BANDERA_G = (SELECT COUNT(*) FROM gruposreserba WHERE EXISTS(SELECT gruposreserba.fecha,gruposreserba.hora FROM gruposreserba WHERE gruposreserba.fecha = FECHA_CLIENTE AND gruposreserba.hora = HORA_CLIENTE AND gruposreserba.deporte = 'gym'));
        
        #si es 0 significa que no hay ninguna reserba hecha ese dia y hora
        IF BANDERA_G = 0 THEN
            #CREO LA RESERBA
            INSERT INTO gruposreserba SET gruposreserba.hora = HORA_CLIENTE,gruposreserba.fecha = FECHA_CLIENTE,gruposreserba.contador = 1,gruposreserba.deporte = 'gym';
            SET ID_RESERBA = (SELECT idGrupo FROM gruposreserba WHERE gruposreserba.fecha = FECHA_CLIENTE AND gruposreserba.hora = HORA_CLIENTE AND gruposreserba.deporte = 'gym');
            #INSERTO EL CLIENTE
        ...
        ELSE
            ...
            SET CONTADOR_R = (SELECT gruposreserba.contador FROM gruposreserba WHERE gruposreserba.hora = HORA_CLIENTE AND gruposreserba.fecha = FECHA_CLIENTE AND gruposreserba.deporte = 'gym');
...
CREATE FUNCTION INSERTAR_CLIENTE_PILETA(EMPLEADO INT (4),NOMBRE_CLIENTE CHAR(15),APELLIDO_CLIENTE CHAR(15),DEPORTE_CLIENTE CHAR(9),FECHA_CLIENTE DATE,HORA_CLIENTE INT(2)) RETURNS VARCHAR(100)
    ...
    DECLARE CONTADOR1 INT(5) DEFAULT (SELECT gruposreserba.contador FROM gruposreserba WHERE gruposreserba.hora = HORA_CLIENTE AND gruposreserba.fecha = FECHA_CLIENTE AND gruposreserba.deporte = 'pileta');
...
WHERE EXISTS(SELECT gruposreserba.fecha,gruposreserba.hora FROM gruposreserba WHERE gruposreserba.fecha = FECHA_CLIENTE AND gruposreserba.hora = HORA_CLIENTE AND gruposreserba.deporte = 'pileta'));
        ...
        IF BANDERA_G = 0 THEN
            #CREO LA RESERBA
            INSERT INTO gruposreserba SET gruposreserba.hora = HORA_CLIENTE,gruposreserba.fecha = FECHA_CLIENTE,gruposreserba.contador = 1,gruposreserba.deporte = 'pileta';
            SET ID_RESERBA = (SELECT idGrupo FROM gruposreserba WHERE gruposreserba.fecha = FECHA_CLIENTE AND gruposreserba.hora = HORA_CLIENTE AND gruposreserba.deporte = 'pileta');
            #INSERTO EL CLIENTE
        ...
        ELSE
            #verifico si todavia no se ha completado las 10 personas
            SET CONTADOR_R = (SELECT gruposreserba.contador FROM gruposreserba WHERE gruposreserba.hora = HORA_CLIENTE AND gruposreserba.fecha = FECHA_CLIENTE AND gruposreserba.deporte = 'pileta');
            #si ya hay 10 personas
            IF CONTADOR_R = 10 THEN
            ...
            ELSE
                ...
                UPDATE gruposreserba
                SET gruposreserba.contador = CONTADOR1 +1
                WHERE  gruposreserba.hora = HORA_CLIENTE AND gruposreserba.fecha = FECHA_CLIENTE AND gruposreserba.deporte = 'pileta';
